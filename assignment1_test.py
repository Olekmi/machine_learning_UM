...
def neural_network (alpha_0, decay_rate, iterations, stop_criterion):
    ...
    for i in range(iterations):
        ...
        # activation_hidden_layer = (tanh(hidden_node_input)+1) /2
        ...
        # output_1 = tanh((np.dot(output_hidden_nodes.T,output_layer_weights)) +1)/2

        delta_3 = (output_1 - input_data)
        delta_2 = np.dot(output_layer_weights, delta_3) * output_hidden_nodes * (1 - output_hidden_nodes)
        # delta_2 = np.dot(output_layer_weights, delta_3)*(1- output_hidden_nodes * output_hidden_nodes)
